...
contract IRewardDistributionRecipient is Ownable, Context {
...
contract FeePool is IRewardDistributionRecipient, ReentrancyGuard {
    ...
    {
        /// Split the contributors and protocol developers fees.
        uint _developerRewards = reward.div(FEE_FACTOR);
        reward = reward.sub(_developerRewards);
        ...
        uint balance = _token1.balanceOf(address(this)).sub(_developerRewards);
        require(rewardRate <= balance.div(rewardsDuration), "notifyRewards: provided reward too high");

        lastUpdateTime = block.timestamp;
        periodFinish = block.timestamp.add(rewardsDuration);

        _token1.safeTransfer(beneficial, _developerRewards);        
